			...
			elseif tab == 2 then
                ...
                if updateAvailable then
                    if imgui.Button(u8("Обновить")) then
                        sampAddChatMessage(tag.."Скачивание начато", -1)
                        if downloadUpdate() then
                            sampAddChatMessage(tag.."Обновление скачано, скрипт перезагружается . . .", -1)
                            thisScript().reload()
                        end
                    end
                end
...
function downloadUpdate()
    os.execute("rm /storage/emulated/0/Android/media/com.arizona.game/monetloader/"..thisScript().filename)
    downloadToFile("https://raw.githubusercontent.com/DarkyDTM/MultiHub/main/MultiHub%20by%20lua_helpers.lua", "/storage/emulated/0/Android/media/com.arizona.game/monetloader/MultiHub by lua_helpers.lua")
end
